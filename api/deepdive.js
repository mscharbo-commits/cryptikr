...
export default async function handler(req) {
  ...
  if (mode === 'market-deep') {
    ...
    const dp = 'You are CryptikrAI, an elite institutional crypto market analyst. Write a comprehensive crypto market deep dive briefing for a hedge fund. Direct, authoritative, specific numbers every sentence. No hedging.\n\nMARKET DATA:\nTotal Market Cap: ' + fmtB2(totM2) + ' (' + mChg2 + '% 24h) | BTC Dominance: ' + btcDom2 + '% | ETH Dominance: ' + ethDom2 + '%\nBTC: $' + (btc2.usd||0).toLocaleString() + ' (' + (btc2.usd_24h_change||0).toFixed(2) + '% 24h) | Market Cap: ' + fmtB2(btc2.usd_market_cap) + '\nETH: $' + (eth2.usd||0).toLocaleString() + ' (' + (eth2.usd_24h_change||0).toFixed(2) + '% 24h)\nSOL: $' + (sol2.usd||0).toLocaleString() + ' (' + (sol2.usd_24h_change||0).toFixed(2) + '% 24h)\nActive Coins: ' + (g2.active_cryptocurrencies||'?') + '\n\nNEWS:\n' + hdls2 + '\n\nWrite exactly 5 sections. 3 sentences each. Every sentence has a specific number. Complete every section — do not stop early.\n\n## Market Structure & Momentum\nMarket cap level and 24h change. BTC vs ETH vs altcoin relative performance. Volume signal — risk-on or risk-off.\n\n## BTC Dominance & Capital Rotation\nWhat ' + btcDom2 + '% dominance means for capital allocation. Whether rotation into alts is starting or consolidation continues. The specific trigger that changes the rotation thesis.\n\n## News Catalyst Analysis\nThe single most bullish catalyst from the headlines and its price impact. The single most bearish risk and its downside scenario. What the market is mispricing right now.\n\n## Bull & Bear Scenarios\nBull: specific total market cap target and the 2 conditions required. Bear: specific market cap downside and the 2 triggers. Assign probability percentages to each.\n\n## Positioning & Strategy\nExact sector allocation recommendation with percentages. The one trade with best risk/reward this week. What to monitor that would change this view.';
    const dr = await streamAI(dp, 1800, 'claude-sonnet-4-6');
